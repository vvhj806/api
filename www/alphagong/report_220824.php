...
if ($method == 'getReport') { // ============================ 인터뷰 종합분석 ============================
    ...
    if (isset($result['user_id'])) { //success
        ...
        echo json_encode(array(
            ...
            'rank' => $rank,
            'point' => $point,
            'point_avg' => 'point_avg', //추가 (면접 종합 접수 - 평균)
            'pick_best' => $pick_best,
            'pick_worst' => $pick_worst,
            'hertz' => 'hertz', //추가 (음성높낮이)
            'dB' => 'dB',    //추가 (목소리크기)
            'facial_analysis' => 'facial_analysis', //추가 (표정분석 - 본인점수)
            'facial_analysis_avg' => 'facial_analysis_avg', //추가 (표정분석 - 평균점수)
            'mbti' => 'mbti', //추가 (mbti)
            'response_reliability ' => '높음 ', //추가 (응답신뢰성)
        ));
    } else if ($result == -10) {
        echo json_encode(array('status' => -10, 'message' => '올바르지 않은 토큰입니다.'));
    } else if ($result == -11) {
    ...
    if (isset($result['user_id'])) { //success
        ...
        for ($i=0; $row1 = mysqli_fetch_assoc($rst1); $i++) {
            ...
            array_push($part_list, array('question' => $question, 'video' => $video, 'point' => $point, 'grade' => $grade, 'grade_chart' => array_values($grade_list), 'rank' => $rank, 'sum' => $sum, 'speech_text' => $row1['speech_text'], 'speech_text_detail' => json_decode($row1['speech_text_detail'], true), 'speech_text_chart' => 'speech_text_chart', 'word_cloud' => 'word_cloud'));
        }
