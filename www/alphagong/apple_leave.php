...
if (!$authorizationCode) {
    echo json_encode(array('status' => 400, 'message' => 'authorizationCode값 없음'));
    return;
}

//엑세스토큰 받기
$teamId = 'DQALHSS7F2';
$clientId = 'kr.bluevisor.Alphagong-iOS';
$keyFileId = '738B4R89BJ';
$keyFileName = 'AuthKey_Alphagong.pem';
$redirectUri = 'https://api.highbuff.com/alphagong/app_leave.php';
...
if ($status_code == 200) {
    ...
    echo json_encode(array('status' => 400, 'message' => 'access_token 구하기 실패'));
    return;
}
...
curl_setopt($ch, CURLOPT_POST, 1);
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($data_revoke));
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
if ($status_code == 200) {
    echo json_encode(array('status' => 200, 'message' => '성공', 'status_code' => $status_code));
} else {
