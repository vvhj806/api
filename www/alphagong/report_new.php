...
if ($method == 'getReport') { // ============================ 인터뷰 종합분석 ============================
    ...
    if (isset($result['user_id'])) { //success
        ...
        //----------------- report_function.php 참고 -------------------
        //----------------- get point avg (전체 평균) -------------------
        $getPointAvg = getPointAvg($conn);

        //----------------- get mbti -------------------
        $getMbti = getMbti($result['user_id'], $conn);

        //----------------- get voice (dB, hz) -------------------
        $getVoice = getVoice($conn, $idx);

        //----------------- 표정분석-본인점수 -------------------
        $getfacialAnalysis = getfacialAnalysis($conn, $idx);

        //----------------- 표정분석-평균점수 -------------------
        $getfacialAnalysisAvg = getfacialAnalysisAvg($conn);

        //----------------- 응답 신뢰성 -------------------
        $getSincerity = getSincerity($conn, $idx);

        //----------------- 지원 직무 -------------------
        $getApplyType = getApplyType($conn, $idx);

        //이상적인 목소리 값
        $ideal_array = array();
        array_push($ideal_array, array("ideal_db" => 65, "ideal_man" => 130, "ideal_woman" => 230));
        ...
        echo json_encode(array(
            ...
            'rank' => $rank,
            'point' => $point,
            'point_avg' => $getPointAvg, //추가 (면접 종합 접수 - 평균)
            'pick_best' => $pick_best,
            'pick_worst' => $pick_worst,
            'voice' => $getVoice, //추가 (음성-dB, hz)
            'facial_analysis' => $getfacialAnalysis, //추가 (표정분석 - 본인점수)
            'facial_analysis_avg' => $getfacialAnalysisAvg, //추가 (표정분석 - 평균점수)
            'mbti' => $getMbti, //추가 (mbti)
            'response_reliability' => $getSincerity, //추가 (응답신뢰성)
            'steps' => $getApplyType, //추가 (지원직무)
            'ideal' => $ideal_array,
        ));
    } else if ($result == -10) {
        echo json_encode(array('status' => -10, 'message' => '올바르지 않은 토큰입니다.'));
    ...
    if (isset($result['user_id'])) { //success
        ...
        for ($i = 0; $row1 = mysqli_fetch_assoc($rst1); $i++) {
            ...
            //면접영상 STT 분석 (단어분포표)
            $getWordStt = getWordStt($conn, $idx);


            array_push($part_list, array('question' => $question, 'video' => $video, 'point' => $point, 'grade' => $grade, 'grade_chart' => array_values($grade_list), 'rank' => $rank, 'sum' => $sum, 'speech_text' => $row1['speech_text'], 'speech_text_detail' => json_decode($row1['speech_text_detail'], true), 'speech_text_chart' => $getWordStt['wordDistribution'], 'word_cloud' => $getWordStt['wordList']));
        }
