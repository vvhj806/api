...
if ($user_key == '' || $client_id == '') {
    ...
    $str = "[API_ERROR]\n경로 : /alphagong/sns/kakao/call_back.php\n에러 : kakao login not found user_key";
    telegram_send($str, "HB_TEST");
    return;
}

$object_sha = sha1($user_key);
$sql1 = "SELECT * FROM iv_sns_member WHERE `provider` = 'kakao' AND user_key = '".$user_key."' AND update_dates != ''"; 
$rst1 = mysqli_query($conn, $sql1);
$row1 = mysqli_fetch_array($rst1);
if ($email == ''){ //GET으로 들어온 이메일이 없다면
    $sql2 = "SELECT email FROM iv_sns_member WHERE `provider` = 'kakao' AND user_key = '".$user_key."'AND email != ''"; 
    $rst2 = mysqli_query($conn, $sql2);
    $row2 = mysqli_fetch_array($rst2);
    $email = $row2["email"];
}

$sql3 = "SELECT * FROM g5_member WHERE mb_id = 'kakao_".$object_sha."'";
...
if (isset($row1)) { //기존 SNS 회원이라면 access_token 전달
    ...
    $sql4 = "INSERT INTO iv_sns_member SET provider = 'kakao', user_key = '".$user_key."', object_sha = '".$object_sha."', email = '".$email."', dates = '".date("Y-m-d H:i:s")."'";
