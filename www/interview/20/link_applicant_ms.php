...
if ($type == 'start') {
    ...
    if (!$rst1 = mysqli_query($conn_iv_15, $sql1)) {
        ...
        $DateTime = date_create($start_date[0]);
        $newStartDate = date_format($DateTime, "Ymd");

        $DateTime = date_create($end_date[0]);
        $newEndDate = date_format($DateTime, "Ymd");
        ...
        if ($row1['ap_personal_q_list']) {
            //1.5 question -> 2.0 iv_question 있는지 확인
            if (strpos($row1['ap_personal_q_list'], ',') !== false) { // , 가 포함됨 / 질문 여러개
                $str_ap_person_q = explode(',', $row1['ap_personal_q_list']);
            } else { // , 가 포함 안됨 / 질문 한개
                $str_ap_person_q[0] = $row1['ap_personal_q_list'];
            }
            $idx_personal = '';
            foreach ($str_ap_person_q as $key => $val) {
                $sql_per = "SELECT q_question FROM question WHERE q_idx = '" . $val . "'";
                $rst_per = mysqli_query($conn_iv_15, $sql_per);
                $row_per = mysqli_fetch_array($rst_per);

                $sql_per2 = "SELECT idx FROM iv_question WHERE que_question = '" . $row_per['q_question'] . "' AND que_type ='B'";
                $rst_per2 = mysqli_query($conn_iv_20, $sql_per2);
                $row_per2 = mysqli_fetch_array($rst_per2);
                $bar_per = ',';

                if ($key == count($str_ap_person_q) - 1) $bar_per = '';

                if ($row_per2['idx'] > 0) {
                    $idx_personal .= $row_per2['idx'] . $bar_per;
                } else {
                    $sql_per3 = "INSERT INTO iv_question (que_question, que_type, delyn, que_reg_date) VALUES 
                                ('{$row_per['q_question']}', 'I','N',NOW())";
                    if (!$rst_per3 = mysqli_query($conn_iv_20, $sql_per3)) {
                        $error_per3 = mysqli_error($conn_iv_20);

                        return_error('003', '111111', $error_per3, $sql_per3);
                        return;
                    } else {
                        $idx_personal .= mysqli_insert_id($conn_iv_20) . $bar_per;
                    }
                }
            }
        } else {
            ...
            if ($row3['idx'] > 0) {
                if ($row3['com_idx'] > 0) {
                    ...
                    if ($i_idx == '1') {
                        ...
                        AND inter_name = '{$row1['i_title']}' AND inter_reg_date = '{$row1['i_date']}'";
                    }
                        ...
                        if ($row4['idx'] > 0) {
                            ...
                            if ($row00211['idx'] > 0 || isset($row00211['idx'])) {
                                ...
                                if ($row0021['idx'] > 0) {
                                    ...
                                    sug_app_personal_q_list = '" . $idx_personal . "'
                                    ";
    ...
    if (!$rst1 = mysqli_query($conn_iv_20, $sql1)) {
        ...
        if (!$rst2 = mysqli_query($conn_iv_15, $sql2)) {
            ...
            $sql3 = "UPDATE applicant SET  ap_status ='A.I. 면접 완료' , ap_ai_result ='{$report_url}'  WHERE ap_idx='{$row2['ap_idx']}'";
