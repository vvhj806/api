...
if (!$code && !$app_code) {
    ...
    header('Location: https://interview.highbuff.com/my/leave/step1');
    return;
}

$teamId = 'DQALHSS7F2';
if ($app_code != '' && $app_code != null) {
...
$redirectUri = 'https://api.highbuff.com/interview/20/leave_auth.php';
if ($app_code != '' && $app_code != null) {
    $authorizationCode = $app_code;
} else {
...
if ($status_code == 200) {
    ...
    if ($app_code != '' && $app_code != null) {
        // header('Location: https://localinterviewr.highbuff.com/sns/apple/web/leave?app=1&at=' . $access_token);
        header('Location: https://interview.highbuff.com/sns/apple/web/leave?app=1&at=' . $access_token);
        //header('Location: https://webtestinterviewr.highbuff.com/sns/apple/web/leave?app=1&at=' . $access_token);
    } else {
        // header('Location: https://localinterviewr.highbuff.com/sns/apple/web/leave?at=' . $access_token);
        header('Location: https://interview.highbuff.com/sns/apple/web/leave?at=' . $access_token);
        //header('Location: https://webtestinterviewr.highbuff.com/sns/apple/web/leave?at=' . $access_token);
    }
} else {
    echo "<script>alert('정상적인 접근이 아닙니다!'); window.close(); opener.location.href='/login.php'; </script>";
    print_r($response);
