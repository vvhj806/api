    ...
    OpenIDAuthorizationCodeInterface
{
    ...
    /**
     * 22.02.10 igg client_id와 client_secret으로  get access token
     * @param string $client_id
     * @return array
     */
    public function getClientAccessToken($client_id = null)
    {
        $stmt = $this->db->prepare($sql = sprintf('SELECT * from %s where client_id = :client_id ORDER BY expires DESC LIMIT 1', $this->config['access_token_table']));

        $stmt->execute(compact('client_id'));
        if ($result = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            $time = time();
            $expire_time = strtotime($result['expires']);
            if ($expire_time < $time) {
                return array('error' => 'Invalid access token', 'access_token' => $result['access_token']);
            } else {
                return array('access_token' => $result['access_token']);
            }
        }

        return array('error' => 'Invalid execute query');
    }

    /**
     * 22.02.16 igg access_token으로 get client_id
     * @param string $access_token으로
     * @return string|bool
     */
    public function getUserId($access_token = null)
    {
        $stmt = $this->db->prepare($sql = sprintf('SELECT * from %s where access_token = :access_token ORDER BY expires DESC LIMIT 1', $this->config['access_token_table']));
     ...
     * @return bool|string
     */
    public function updateAccessToken($access_token, $client_id, $user_id, $expires, $scope = null)
    {
        // convert expires to datestring
        $expires = date('Y-m-d H:i:s', $expires);
        ...
        // if it exists, update it.
        if ($this->getAccessToken($access_token)) {
            $new_access_token = $this->generateAccessTokenPublic();
            $stmt = $this->db->prepare(sprintf('UPDATE %s SET client_id=:client_id, expires=:expires, user_id=:user_id, scope=:scope, access_token=:new_access_token where access_token=:access_token', $this->config['access_token_table']));

            $stmt->execute(compact('access_token', 'client_id', 'user_id', 'expires', 'scope', 'new_access_token'));
    ...
    /**
     * 22.02.23 igg 액세스 토큰 재발행 
     * @param string $access_token
     * @return bool|string
     */
    public function updateAccessTokenSingle($access_token)
    {
        // if it exists, update it.
        if ($this->getAccessToken($access_token)) {
            $expires = date('Y-m-d H:i:s', strtotime("+86400 seconds")); // //expire 1day

            $new_access_token = $this->generateAccessTokenPublic();
            $stmt = $this->db->prepare(sprintf('UPDATE %s SET expires=:expires, access_token=:new_access_token where access_token=:access_token', $this->config['access_token_table']));

            $stmt->execute(compact('access_token', 'expires', 'new_access_token'));
            return $new_access_token;
        } else {
            return false;
        }
    ...
    public function deleteAccessToken($access_token)
    {
        ...
        $stmt->execute(compact('access_token', 'expires'));

        return $stmt->rowCount() > 0;
    }

    private function generateAccessTokenPublic()
    {
